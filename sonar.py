...
def enterPlayerMove():
    # Let thee player type in their move. Return a two-item list of int xy coordinates.
    ...
    while True:
        ...
        print('Enter a number from 0 to 59, a space, the a number from 0 to 14.')
...
def showInstructions():
    ...
    The treasur chessts don't move around. Sonar devices can detect treasure
    ...
    the location of the chest, an it will be collected. The sonar device will
    remain there.
...
while True:
    ...
    while sonarDevices > 0:
        ...
        if sonarDevices > 1: extraSonar = 's'
        else: extraSonar = ''
        if len(theChests) > 1: extraChest = 's'
        else: extraChest = ''
        print('You have %s sonar devices %s left. %s treasure chest %s remaining.' %(sonarDevices, extraSonar, len(theChests), extraChest))
    ...
    if sonarDevices == 0:
        print('We\'ve run out of sonar devices! Now we have to turn the ship around and ahead')
